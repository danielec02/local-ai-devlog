...
1. Avviare temporaneamente un container SearXNG per generare i file:
   ...
   ```
2. Una volta individuato il file `settings.yml` nella cartella `/etc/searxng`, sostituirlo con quello di questa cartella o semplicemente aggiungere `- json` sotto i `formats`.
3. Riavviare il container in modo da montare il file modificato.

### Configurazione finale OpenClaw
Una volta che il container è attivo (ed espone JSON sulla porta 8888), procedere con il setup lato OpenClaw:
